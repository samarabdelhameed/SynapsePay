...
pub mod synapsepay_scheduler {
    ...
    /// Initialize scheduler with vault authority
    pub fn initialize_scheduler(ctx: Context<InitializeScheduler>) -> Result<()> {
        instructions::initialize_scheduler::handler(ctx)
    }
